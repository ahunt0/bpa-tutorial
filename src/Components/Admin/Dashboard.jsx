	...
	<Card className="p-4 w-96">
		<CardHeader>
			<div className="flex flex-col text-default-800">
				<Skeleton className="absolute top-0 right-0 mt-4 mr-4 bg-red-500/20 rounded-full p-2">
					<div className="w-8 h-8 bg-default-300"></div>
				</Skeleton>
...
export default function Dashboard() {
	...
	useEffect(() => {
		...
		const loadData = async () => {
			setIsLoading(true);
			await Promise.all([fetchData("total", setTotalUsers), fetchData("today", setNewUsersToday)]);
			setIsLoading(false);
		};
